...
window.ChatLabWebRtcTelemetry = (() => {
  ...
  async function snapshot(peerConnection) {
    ...
    return {
      capturedAtUtc: new Date().toISOString(), connectionState: peerConnection.connectionState ?? 'unknown', iceConnectionState: peerConnection.iceConnectionState ?? 'unknown', signalingState: peerConnection.signalingState ?? 'unknown',
  ...
  function start(peerConnection, { endpoint, intervalMs = 1000, onError = console.warn } = {}) {
    ...
    const send = async () => {
      if (stopped) return;
      try {
        const response = await fetch(endpoint, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(await snapshot(peerConnection)) });
